# QA Scorecard — talking_slow

- **Reviewer:** qa
- **Date:** 2026-07-25
- **Review round:** 1 (lip-sync Tier-2 batch, PERC_LOSSLESS+gopsize250)
- **Duration/loop:** 8s
- **Evidence:** previews/talking_slow/ (mp4, strip, stills, meta.json), qa/reports/talking_slow_inspection.png + _metrics.json (inspect_clip: 0 flags), qa/reports/curve_audit_lipsync.json (jaw rhythm/overlap, viseme set, blink schedule, seams).
...
| 2 | Timing & spacing | 9 | non-metronomic: interval CV=0.89 (incl. pauses), within-phrase syllables varied; pause structure present (long jaw gaps + breath) — NOT a flat flap. |
| 3 | Naturalness | 9 | jaw coarticulation overlap = 19% of peak within-phrase (jaw does NOT fully close between adjacent syllables), closes to ~0 only at phrase boundaries. deliberate: sparse peaks with long 64-105f pauses. |
| 4 | Facial aliveness | 9 | expressive speech: jaw amp 0.190 + visemes + brow/mouth accents; distinct per-clip character. |
