# QA Scorecard — talking_laughing

- **Reviewer:** qa
- **Date:** 2026-07-25
- **Review round:** 1 (lip-sync Tier-2 batch, PERC_LOSSLESS+gopsize250)
- **Duration/loop:** 8s
- **Evidence:** previews/talking_laughing/ (mp4, strip, stills, meta.json), qa/reports/talking_laughing_inspection.png + _metrics.json (inspect_clip: 0 flags), qa/reports/curve_audit_lipsync.json (jaw rhythm/overlap, viseme set, blink schedule, seams).
...
| 2 | Timing & spacing | 9 | non-metronomic: interval CV=0.65 (incl. pauses), within-phrase syllables varied; pause structure present (long jaw gaps + breath) — NOT a flat flap. |
| 3 | Naturalness | 9 | jaw coarticulation overlap = 26% of peak within-phrase (jaw does NOT fully close between adjacent syllables), closes to ~0 only at phrase boundaries. big open laughing mouth: jaw 0.44 (largest) + Mouth_Smile 0.72, teeth reveal (strip); clavicle/spine laugh-shake owned. |
| 4 | Facial aliveness | 9 | expressive speech: jaw amp 0.440 + visemes + brow/mouth accents; distinct per-clip character. |
...
| 7 | Loop seamlessness | 9 | seam vdiff=0.0000 tdiff=0.0000 (all channels). talking_laughing's author-flagged clavicle seam confirmed auto-corrected (0.0000). |
